...
import {
  createCredential, createJob, listCredentials,
...
// 凭据长期保存：一年，期间任务可重复使用该授权；可随时在「我的凭据」删除
const CREDENTIAL_TTL_SECONDS = 365 * 24 * 60 * 60;
...
export default function Evaluations() {
  ...
  useEffect(() => {
    ...
    // 已托管的学校统一认证（school）与旧版评课专用凭据都可用，优先统一认证
    listCredentials()
      .then((list) => {
        const arr = Array.isArray(list) ? list : (list?.items || []);
        const school = arr.find((c) => c.purpose === "school" && c.status === "active");
        const legacy = arr.find((c) => c.purpose === "evaluation" && c.status === "active");
