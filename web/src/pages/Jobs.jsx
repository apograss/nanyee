...
import { ListTodo, AlertTriangle, Search, RefreshCw } from "lucide-react";
...
export default function Jobs() {
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-5" data-component="JobsPage" data-od-id="jobs">
      <div className="flex flex-col gap-1">
        <div className="text-[11px] tracking-[0.1em] uppercase text-[var(--muted)]">凭据与任务</div>
        <h1>任务中心</h1>
        <p className="text-[var(--muted)] text-sm prose-body">在这里查看你的所有任务：排队、执行、重试、完成或失败。需要你手动确认的任务会单独标出。</p>
      </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col divide-y divide-border">
              {list.map((j) => {
                ...
                return (
                  <Link to={`/jobs/${j.id}`} key={j.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0 hover:bg-[var(--seed-surface-2)] -mx-2 px-2 rounded-[var(--radius-sm)]" data-component="JobRow" data-od-id={j.id}>
                    <StatusBadge status={j.state} />
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-medium truncate">{summarize(j)}</div>
