...
export default function JobDetail() {
  ...
  if (error && !job) {
    return (
      <div className="max-w-3xl mx-auto flex flex-col gap-5" data-component="JobDetailPage" data-od-id="job-detail">
        <Link to="/jobs" className="kicker inline-flex items-center gap-1.5 hover:text-foreground transition-colors no-underline"><ArrowLeft className="w-3.5 h-3.5" /> 返回任务列表</Link>
  ...
  return (
    <motion.div
      ...
      data-component="JobDetailPage"
      data-od-id="job-detail"
    >
      {/* ---------- 编辑风页头 ---------- */}
      <motion.div variants={fadeUp} className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <span className="kicker"><strong>Job Detail</strong> — 任务详情</span>
          <span className="rule-line flex-1" />
          <Link to="/jobs" className="kicker inline-flex items-center gap-1.5 hover:text-foreground transition-colors no-underline">
            <ArrowLeft className="w-3.5 h-3.5" /> 返回任务列表
          </Link>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="display-lede">{summarize(job)}</h1>
          <StatusBadge status={job.state} />
        </div>
        <p className="text-[var(--muted)] text-sm prose-body">
          任务详情
          <span className="accent-en ml-2 normal-case tracking-normal text-[13px]">status, payload &amp; receipt.</span>
        </p>
      </motion.div>
      ...
      {job.tool_id === "evaluation" && Array.isArray(job.receipt?.logs) && (
        <motion.div variants={fadeUp}>
          <Card data-component="EvaluationLog">
            <CardHeader>
              <div className="kicker"><strong>Log</strong></div>
      ...
      <motion.div variants={fadeUp}>
        <Card>
          <CardHeader>
            <div className="kicker"><strong>Meta</strong></div>
      ...
      <motion.div variants={fadeUp}>
        <Card data-component="PayloadCard" data-od-id="payload">
          <CardHeader>
            <div className="kicker"><strong>Payload</strong></div>
      ...
      {cancelError && (
        <motion.div variants={fadeUp}>
          <Alert variant="warning" title="取消失败">{cancelError}</Alert>
        </motion.div>
      )}
