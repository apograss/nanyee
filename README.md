...
- 课表查询、聚合、ICS 与 WakeUp 文件导出，成绩解析与汇总领域链路。
- 在线确认型选课：刷新当前可选列表后单次提交，结果未知时禁止重试。
- 在线确认型评课：读取待评、生成一次性选项草稿、完整选择后单次提交。
- 学习舱预约：18 个舱位优先级、开放/冻结时段判断、加密学校凭据与 OCR 登录。
- 群报数即时 Token 校验、表单同步、受限图片上传、字段自动回填预览与托管预约提交。
- 托管凭据信封加密：每条凭据独立 AES-256-GCM 数据密钥，可用 Azure Key Vault RSA-OAEP-256 或本地主密钥包裹。
- 持久任务：幂等键、计划时间、租约、心跳、取消、有限重试和人工核验状态。
